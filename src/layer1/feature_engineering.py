"""
Layer 1 — Feature Engineering  (standalone, no src imports)
===========================================================
Fix: changed  from src.layer1.attention_pooling import attention_summary
          to  from attention_pooling import attention_summary
"""
...
from attention_pooling import attention_summary   # same-folder import
...
def build_features(obs: pd.DataFrame, prelayer: dict, config: dict) -> tuple[pd.DataFrame, dict]:
    ...
    sl_context = (
        obs.groupby(['service_line', 'week_index', 'day_of_week'])
    ...
    if exc:
        e = pd.DataFrame(exc)
        if 'dominant_provider_id' in e.columns and 'exception_rate' in e.columns:
            e_prov = (
                e.groupby('dominant_provider_id')['exception_rate']
                 .mean()
                 .rename('provider_exception_rate')
                 .reset_index()
            )
            features = features.merge(
                e_prov,
                left_on='provider_id',
                right_on='dominant_provider_id',
                how='left',
            )
    ...
    return (
        ...
        {'wq_case': wq_case, 'wq_turn': wq_turn},
    )
